		...
		for value_len, value_set in dict_items:
			for value in value_set:
				if i >= start and i < end:
					...
					if 0 == (i % 1000):
						logging.info("\t analysis 20 progress thread " + str(thread_id) + ": " + str(i) + " - " + str(count))
				i += 1


		q.put(count)
		logging.info("\t analysis 20 thread " + str(thread_id) + " done")
...
def run(data):
	...
	for record in data:
		value = record["abstract"].lower()
		value_len = len(value)
		
		len_dict.setdefault(value_len, set())
		len_dict[value_len].add(value)
	...
	logging.info("\t\t analysis 20: " + str(count))

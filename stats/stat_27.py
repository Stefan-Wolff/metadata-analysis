...
def run_comparison(doi_names):
	...
	for doi, group in doi_names.items():							# collect all author groups
		...
		i += 1
		if 0 == (i % 1000):
			logging.info("\t analysis 27 group progress: " + str(i))
	...
	for group_hash, group_info in groups.items():					# log sizes of found author groups
		...
		i += 1
		if 0 == (i % 1000):
			logging.info("\t analysis 27 progress thread: " + str(i))


	logging.info("\t analysis 27 done: " + str(count))
...
def run(data):
	...
	for record in data:
		...
		for author in record["authors"]["authors"]:
			publ_authors.add(str(author["id"]))
	...
	# write results
	out_path = os.path.join(os.path.dirname(__file__), '../logs/stats_27.csv')
	writeCSV(out_path, count)


	logging.info("\t analysis 27 done")

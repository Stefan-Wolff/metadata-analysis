...
def run_comparison(doi_names, name_doi_id, namesakes):
	...
	for doi, group in doi_names.items():							# collect all author groups
		...
		i += 1
		if 0 == (i % 1000):
			logging.info("\t analysis 30 group progress: " + str(i))
	...
	logging.info("\t analysis 30 done: " + str(count))
...
def run(data):
	...
	# write results
	out_path = os.path.join(os.path.dirname(__file__), '../logs/stats_30.csv')
	writeCSV(out_path, count)


	logging.info("\t analysis 30 done")
